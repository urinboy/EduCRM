...
<div class="d-flex justify-content-center">
    <div class="card shadow bg-white w-100">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route($__env->yieldContent('name') . '.update', $currency->id) }}">
                @csrf
                @method('PUT')
                <div class="row p-2">
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="form-group mt-2">
                            ...
                            <input type="text" id="name" name="name" placeholder="Name" class="form-control w-100 @error('name') is-invalid @enderror" required value="{{ old('name', $currency->name) }}">
                            @error('name')
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="form-group mt-2">
                            ...
                            <input type="text" id="code" name="code" placeholder="Code" class="form-control w-100 @error('code') is-invalid @enderror" required value="{{ old('code', $currency->code) }}">
                            @error('code')
                                <span class="invalid-feedback d-block" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 mt-2 d-flex justify-content-end">
                        <button type="submit" class="shadow btn btn-success btn-sm mt-2 mb-3 px-5 py-2">
                            <i class="fa-solid fa-floppy-disk"></i> {{ __("crud.btn_update") }}
                        </button>
                    </div>
                </div>
            </form>
